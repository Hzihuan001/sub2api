...
- L1 使用标准“分组管理”页面；管理员可以自行创建、复制、编辑、排序、停用和删除销售分组，并配置分组倍率、说明、用户倍率覆盖、限额和模型规则。
- “上游管理”页面允许把站点所有者预先配置的上游线路分配到一个或多个同协议销售分组；该操作在后端只允许修改上游账号的 `group_ids`。
- L1 管理员不能新增、导入、修改、复制或删除上游账号。
- L1 管理员不能借分组分配修改上游账号名称、密钥、地址、成本倍率、状态、并发、优先级或其他账号字段。
- L1 管理员不能配置代理 IP、执行 OAuth 接入、安装/修改插件或通过整库恢复引入其他上游。
- L1 内置更新与回滚接口只读，防止官方升级覆盖“仅 Moshu”约束。
- 当前 Moshu 分组密钥仍由站点所有者配置；L1 管理员不能看到密钥明文。

当前单一上游线路示例：Moshu 分组 `GPT-企业` 的成本倍率为 `0.35`。L1 可以将该线路分配给自己创建的一个或多个 OpenAI 销售分组，每个销售分组独立设置倍率。L1 用户消费记入 L1，真实上游成本记入 Moshu。
