...
const libraryUserID = useAuthStore().user?.id ?? 0
...
async function generate(): Promise<void> {
  ...
  try {
    ...
    if (stored.length) {
      try {
        await saveStoredStudioImages(stored, libraryUserID)
      } catch {
        storageFailed = true
...
async function loadLibrary(): Promise<void> {
  ...
  try {
    const images = await listStoredStudioImages(libraryUserID)
...
async function deleteItem(item: GalleryItem): Promise<void> {
  if (item.persisted) {
    await deleteStoredStudioImage(item.id, libraryUserID)
    await loadLibrary()
  } else {
...
async function confirmClearLibrary(): Promise<void> {
  ...
  try {
    await clearStoredStudioImages(libraryUserID)
    await loadLibrary()
  } catch (error) {
...
function downloadItem(item: GalleryItem): void {
  ...
  anchor.download = `image-${new Date(item.createdAt).toISOString().replace(/[:.]/g, '-')}.${extensionFor(item)}`
  anchor.rel = 'noopener'
  anchor.click()
}
