...
onMounted(async () => {
  ...
  await Promise.all([
    fetchLogs(),
    fetchHealth(),
    authStore.isAdmin ? loadRuntimeConfig() : Promise.resolve()
  ])
})
</script>
...
<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    ...
    <div v-if="authStore.isAdmin" class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
        ...
        <label class="text-xs text-gray-600 dark:text-gray-300">
          ...
          <label class="text-xs text-gray-600 dark:text-gray-300">
            ...
            <LogRetentionSelect v-model="runtimeConfig.retention_days" :label="t('admin.ops.systemLogs.retentionDays')" />
          ...
          <label class="text-xs text-gray-600 dark:text-gray-300">
            ...
            <LogRetentionSelect v-model="runtimeConfig.request_retention_days" :label="t('admin.ops.systemLogs.requestRetentionDays')" allow-forever />
    ...
    <div class="mb-3 flex flex-wrap gap-2">
      ...
      <button v-if="authStore.isAdmin" type="button" class="btn btn-danger btn-sm" @click="cleanupCurrentFilter">{{ t('admin.ops.systemLogs.cleanCurrentFilters') }}</button>
