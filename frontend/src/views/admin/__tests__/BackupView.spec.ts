...
// Keep the aliases used by the archive-specific regression cases tied to the
// shared mock object.  These tests used to destructure each mock separately;
// after the v0.2.8 merge the mocks are grouped under apiMocks, so leaving the
// old names unresolved makes Vitest fail before mounting the component.
...
describe('BackupView', () => {
  ...
  it('归档删除需要单独确认，有限归档不会显示永不过期', async () => {
    ...
    expect(wrapper.find('.confirm-dialog-stub').text()).toContain('admin.backup.archive.deleteConfirm')
    expect(deleteBackup).not.toHaveBeenCalled()
    await wrapper.find('.confirm-dialog-stub button:last-child').trigger('click')
    await flushPromises()
