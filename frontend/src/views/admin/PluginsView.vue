...
type PendingPluginAction =
  | { kind: "enable-untested"; plugin: PluginInstallation }
  | { kind: "disable"; plugin: PluginInstallation }
  | { kind: "uninstall"; plugin: PluginInstallation };
...
const confirmDialogTitle = computed(() => {
  ...
  if (pendingConfirm.value?.kind === "uninstall") return t("admin.plugins.uninstall");
  return t("admin.plugins.enable");
});

const confirmDialogMessage = computed(() => {
  if (pendingConfirm.value?.kind === "disable") return t("admin.plugins.confirmDisable");
  if (pendingConfirm.value?.kind === "uninstall") return t("admin.plugins.confirmUninstall");
  return t("admin.plugins.confirmUntested");
});
...
const confirmDialogDanger = computed(
  () => pendingConfirm.value?.kind === "disable" || pendingConfirm.value?.kind === "uninstall",
);
...
async function enablePlugin(plugin: PluginInstallation): Promise<void> {
  if (!plugin.compatibility.tested) {
    pendingConfirm.value = { kind: "enable-untested", plugin };
    return;
  }
  await performEnablePlugin(plugin, false);
}

async function performEnablePlugin(
...
async function disablePlugin(plugin: PluginInstallation): Promise<void> {
  pendingConfirm.value = { kind: "disable", plugin };
}

async function performDisablePlugin(plugin: PluginInstallation): Promise<void> {
  busyID.value = plugin.id;
  try {
...
async function uninstallPlugin(plugin: PluginInstallation): Promise<void> {
  pendingConfirm.value = { kind: "uninstall", plugin };
}

async function performUninstallPlugin(plugin: PluginInstallation): Promise<void> {
  busyID.value = plugin.id;
  try {
...
async function confirmPendingAction(): Promise<void> {
  ...
  if (action.kind === "enable-untested") {
    await performEnablePlugin(action.plugin, true);
  } else if (action.kind === "disable") {
    await performDisablePlugin(action.plugin);
  } else {
    await performUninstallPlugin(action.plugin);
  }
}
