...
const pendingRemoveDeleteArchived = ref(false)
...
function removeBackup(id: string) {
  // Monthly archives are protected by a separate confirmation because the
  // delete request must explicitly opt in to removing the archive policy.
  const record = backups.value.find(item => item.id === id)
  pendingRemoveId.value = id
  pendingRemoveDeleteArchived.value = Boolean(record?.monthly_archive)
  removeDialogMessage.value = pendingRemoveDeleteArchived.value
    ? t('admin.backup.archive.deleteConfirm')
    : t('admin.backup.actions.deleteConfirm')
  showRemoveDialog.value = true
}
async function confirmRemove() {
  showRemoveDialog.value = false
  const deleteArchived = pendingRemoveDeleteArchived.value
  pendingRemoveDeleteArchived.value = false
  try {
    await adminAPI.backup.deleteBackup(pendingRemoveId.value, deleteArchived)
