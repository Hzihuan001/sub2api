export default {
  promptAudit: {
    title: '提示词审计',
    description: '可选择仅在本机异步记录最新用户输入，或通过 Qwen3Guard 节点进行风险审计；完整内容仅供管理员复核。',
    ...
    mode: { off: '已关闭', capture_only: '仅记录提示词', async_audit: '异步只审计', blocking: '同步审计并阻止' },
    status: { disabled: '未启用', running: '运行中', degraded: '降级', error: '错误', healthy: '健康', failed: '失败', stale: '心跳过期' },
    decisions: { unreviewed: '未审查', pass: '通过', flag: '标记', critical: '严重' },
    ...
    recording: {
      ...
      cleanupNow: '立即按策略清理', cleanupConfirmTitle: '立即清理提示词记录？', cleanupConfirmMessage: '将按当前保留天数和容量上限永久删除过期或超额的“仅记录”事件。Guard 审计事件不受影响。',
    },
    saveBar: { mode: '模式', blockingLatestTurnOnly: '仅审最新输入和上一轮输出', storePass: '保存安全事件', dirty: '有未保存的更改', synced: '配置已同步' },
    blockingConfirm: {
      title: '开启同步阻止？',
    ...
    events: {
      title: '提示词事件', description: '按身份、入口、来源、Hash 和时间查看记录，详情中可查看完整提示词。', captureMode: '记录来源', captureOnly: '仅记录', guardAudit: 'Guard 审计', decision: '判定', risk: '风险等级', endpoint: '入口', groupId: '分组 ID', userId: '用户 ID', apiKeyId: 'API Key ID', keyword: '关键词',
    ...
    messages: { saved: '提示词配置已保存，明文 API Key 状态已清除。', probeSucceeded: '审计节点连接正常。', deleted: '已删除 {count} 条提示词事件。', cleanupCompleted: '清理完成，删除 {count} 条仅记录事件。', exported: '已导出 {count} 条提示词事件。', exportTruncated: '已导出前 {count} 条；结果超过单次 10,000 条上限，请缩小筛选范围后继续导出。' },
    errors: {
      loadConfig: '无法加载提示词配置。', loadRuntime: '无法加载提示词运行态。', loadGroups: '无法加载分组列表。', loadEvents: '无法加载提示词事件。', loadDetail: '无法加载事件详情。', saveConfig: '配置保存失败。', probe: '节点探测失败。', cleanup: '提示词记录清理失败。', export: '提示词事件导出失败。', delete: '事件删除失败。', previewDelete: '无法生成删除预览，请检查时间范围。', deleteConfirmation: '删除确认无效或已过期，请重新预览。',
      ...
      prompt_recording_mode_conflict: '仅记录模式与 Guard 审计不能同时开启。', prompt_recording_invalid_retention: '保留天数必须在 1–365 天之间。', prompt_recording_invalid_storage: '容量上限必须在 16–10240 MB 之间。',
    },
  },
}
