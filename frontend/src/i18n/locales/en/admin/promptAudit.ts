export default {
  promptAudit: {
    title: 'Prompt Audit',
    description: 'Record the latest user input locally without a Guard node, or audit it through Qwen3Guard. Full content is available to administrators only.',
    ...
    mode: { off: 'Off', capture_only: 'Record prompts only', async_audit: 'Async audit only', blocking: 'Synchronous audit and block' },
    ...
    recording: {
      ...
      cleanupNow: 'Clean up now', cleanupConfirmTitle: 'Clean up prompt records now?', cleanupConfirmMessage: 'This permanently removes expired or excess capture-only events using the current retention and size limits. Guard audit events are unaffected.',
    },
    saveBar: { mode: 'Mode', blockingLatestTurnOnly: 'Only latest input and prior output', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' },
    blockingConfirm: {
    ...
    events: {
      title: 'Prompt events', description: 'Review records by identity, route, source, hash, and time; details show the full prompt.', captureMode: 'Source', captureOnly: 'Record only', guardAudit: 'Guard audit', decision: 'Decision', risk: 'Risk level', endpoint: 'Endpoint', groupId: 'Group ID', userId: 'User ID', apiKeyId: 'API Key ID', keyword: 'Keyword',
    ...
    messages: { saved: 'Prompt configuration saved; plaintext API Key state was cleared.', probeSucceeded: 'The audit node is reachable.', deleted: 'Deleted {count} prompt events.', cleanupCompleted: 'Cleanup completed; deleted {count} capture-only events.', exported: 'Exported {count} prompt events.', exportTruncated: 'Exported the first {count} events. Narrow the filters to export beyond the 10,000-event limit.' },
    errors: {
      loadConfig: 'Unable to load prompt configuration.', loadRuntime: 'Unable to load Prompt Audit runtime.', loadGroups: 'Unable to load groups.', loadEvents: 'Unable to load prompt events.', loadDetail: 'Unable to load event details.', saveConfig: 'Unable to save the configuration.', probe: 'Node probe failed.', cleanup: 'Unable to clean up prompt records.', export: 'Unable to export prompt events.', delete: 'Unable to delete events.', previewDelete: 'Unable to create a deletion preview. Check the time range.', deleteConfirmation: 'The deletion confirmation is invalid or expired. Preview again.',
      ...
      prompt_recording_mode_conflict: 'Capture-only mode and Guard audit cannot be enabled together.', prompt_recording_invalid_retention: 'Retention must be between 1 and 365 days.', prompt_recording_invalid_storage: 'The storage limit must be between 16 and 10240 MB.',
    },
  },
}
