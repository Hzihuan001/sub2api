<template>
  <div class="flex flex-wrap gap-2">
    <button
      v-for="preset in CURSOR_BASE_URL_PRESETS"
      :key="preset.url"
      type="button"
      data-testid="cursor-base-url-preset"
...
import { CURSOR_BASE_URL_PRESETS, type CursorBaseUrlPreset } from './credentialsBuilder'

// Cursor 快捷端点：点击把预设地址填入调用方的输入框。
...
const presetLabel = (preset: CursorBaseUrlPreset) =>
  preset.label ?? t(`admin.accounts.cursorCustomBaseUrl.presets.${preset.labelKey}`)
