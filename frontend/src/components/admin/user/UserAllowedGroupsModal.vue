...
const load = async () => {
  ...
  loading.value = true
  try {
    // Use the paginated endpoint so the modal is not limited by the default
    // active-group list (and so loading failures remain observable to the UI).
    const res = await adminAPI.groups.list(1, 1000)
    const allGroups = res.items
    // 只显示标准类型且活跃的分组
