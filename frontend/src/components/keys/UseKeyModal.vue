...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready Moshu setups.
  ...
  const configContent = `# Grok Build CLI → Moshu Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for Moshu Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches Moshu).
...
description = "Grok 4.5 via Moshu (Responses)"
...
# Imagine tools: model IDs go to Moshu media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set MOSHU_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:MOSHU_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export MOSHU_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → Moshu Grok group
...
# Grok/Moshu path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const envContent = isWindows
    ? `$env:MOSHU_API_KEY="${apiKey}"`
    : `export MOSHU_API_KEY="${apiKey}"`

  const configContent = `# Codex CLI -> Moshu ${label} group
model_provider = "moshu"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at Moshu OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via Moshu'
