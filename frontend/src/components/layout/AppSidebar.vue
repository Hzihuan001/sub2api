...
// Operators get a recursive menu filter. A parent with children is retained
// only when at least one child remains visible; direct URL access is guarded by
// the router/backend as well. The literal permission check is intentionally
// kept here as the single UI policy boundary for future menu entries.
// (Equivalent to `item.permission && authStore.can(item.permission)`.)
function applyOperatorPermissions(items: NavItem[]): NavItem[] {
  ...
  return items.reduce<NavItem[]>((out, item) => {
    if (!item.permission && !item.children) return out
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/prompt-rules', label: t('nav.promptRules'), icon: PromptIcon, permission: 'promptRules' },
    ...
    { path: '/admin/promo-codes', label: t('nav.promoCodes'), icon: GiftIcon, hideInSimpleMode: true, permission: 'promoCodes' },
    {
...
watch(
  ...
  (v) => {
    ...
    if (isAdmin.value) {
      ...
      adminSettingsStore.fetchOpsCapabilities(true)
    }
  },
  { immediate: true }
)
