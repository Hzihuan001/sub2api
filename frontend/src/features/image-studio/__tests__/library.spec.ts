...
import { base64ImageToBlob, chooseStudioImagesToDelete, studioLibraryName } from '../library'

describe('image studio local library', () => {
  it('isolates image history when different users share a browser', () => {
    expect(studioLibraryName(1)).not.toBe(studioLibraryName(2))
    expect(studioLibraryName(1)).toBe(studioLibraryName(1))
    expect(() => studioLibraryName(0)).toThrow()
  })
