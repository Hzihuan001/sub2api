...
function openLibrary(userID: number): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(studioLibraryName(userID), databaseVersion)
...
export async function listStoredStudioImages(userID: number): Promise<StoredStudioImage[]> {
  const database = await openLibrary(userID)
  try {
...
export async function saveStoredStudioImages(images: StoredStudioImage[], userID: number): Promise<string[]> {
  if (images.length === 0) return []
  let database = await openLibrary(userID)
  try {
  ...
  const allImages = await listStoredStudioImages(userID)
  const idsToDelete = chooseStudioImagesToDelete(allImages)
  if (idsToDelete.length === 0) return []
  database = await openLibrary(userID)
  try {
    const transaction = database.transaction(storeName, 'readwrite')
...
export async function deleteStoredStudioImage(id: string, userID: number): Promise<void> {
  const database = await openLibrary(userID)
  try {
    const transaction = database.transaction(storeName, 'readwrite')
...
export async function clearStoredStudioImages(userID: number): Promise<void> {
  const database = await openLibrary(userID)
  try {
    const transaction = database.transaction(storeName, 'readwrite')
