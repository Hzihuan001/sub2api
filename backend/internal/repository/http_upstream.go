...
const (
	...
	// OpenAI upstreams keep the shorter probe interval used by the original
	// gateway.  This detects a dead proxy/NAT connection quickly without
	// imposing the long-stream policy on interactive OpenAI requests.
...
// Keeping the cancel function attached to the response body is important for
// long-running SSE requests: when a caller stops reading, the transport is
// released immediately and the pooled connection cannot poison the next
// request.
...
func (b *cancelOnCloseBody) Close() error {
	b.once.Do(func() {
		// Cancel before closing, including before closing a decompressor. This
		// prevents an early close racing with an active read from leaving a
		// waiter behind in the HTTP transport.
		b.cancel()
		b.readMu.Lock()
		defer b.readMu.Unlock()
		b.closed = true
...
func enableHTTP2KeepAlive(transport *http.Transport, protocolModes ...string) (*http2.Transport, error) {
	...
	if h2 != nil {
		...
		protocolMode := ""
		if len(protocolModes) > 0 {
			protocolMode = protocolModes[0]
		}
