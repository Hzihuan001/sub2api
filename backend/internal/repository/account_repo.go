...
func lockAndMergeAccountProbeExtra(
	...
	// The OpenCode Go managed-state columns were added after the original
	// probe snapshot query.  Keep the scanner tolerant of older test doubles
	// and database views that still expose the original nine columns; a missing
	// optional column simply means there is no managed OpenCode state to merge.
	scanTargets := []any{
		&identityUnchanged,
	...
	if columns, columnsErr := rows.Columns(); columnsErr != nil {
