...
func RegisterGatewayRoutes(
	...
	resellerProtocol := func(c *gin.Context) { c.Next() }
	if h.Reseller != nil {
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, resellerProtocol, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, resellerProtocol, compositeTarget, requireGroupAnthropic)
	{
	...
	// Antigravity 模型列表
	r.GET("/antigravity/models", gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, resellerProtocol, requireGroupAnthropic, h.Gateway.AntigravityModels)
