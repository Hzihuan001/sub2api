...
func TestMoshuOnlyAccountGuard(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "read-only model probe remains available", method: http.MethodPost, path: "/api/v1/admin/accounts/1/models/probe-upstream", wantStatus: http.StatusNoContent},
		{name: "model mapping sync remains blocked", method: http.MethodPost, path: "/api/v1/admin/accounts/1/models/sync-upstream", wantStatus: http.StatusForbidden},
		{name: "invalid probe account path is blocked", method: http.MethodPost, path: "/api/v1/admin/accounts/data/models/probe-upstream", wantStatus: http.StatusForbidden},
