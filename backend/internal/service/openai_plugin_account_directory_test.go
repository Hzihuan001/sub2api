...
func TestAccountReadableSnapshot_DenylistTripwire(t *testing.T) {
	...
	safeToExpose := map[string]struct{}{
		...
		// Kiro quota/runtime state is operational metadata, not a credential. It is
		// safe for the plugin-readable snapshot and helps plugins avoid probing an
		// account whose quota is exhausted or temporarily unavailable.
