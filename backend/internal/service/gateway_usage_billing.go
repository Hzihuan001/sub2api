...
// recordUsageOpts carries custom billing safeguards that are independent of
// the upstream catalog-driven long-context pricing policy.
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	pricingAt time.Time,
	options ...*recordUsageOpts,
) *CostBreakdown {
	opts := firstRecordUsageOpts(options)
...
func firstRecordUsageOpts(options []*recordUsageOpts) *recordUsageOpts {
	if len(options) == 0 {
		return nil
	}
	return options[0]
