...
func TestOpenAIProxyStreamCircuitScopesBlocksByPlatform(t *testing.T) {
	...
	require.True(t, tripped)
	require.True(t, circuit.isBlockedForPlatform(7, PlatformDeepseek, base.Add(time.Second)))
	require.False(t, circuit.isBlockedForPlatform(7, PlatformOpenAI, base.Add(time.Second)))
	require.Equal(t, 1, circuit.activeBlockCountForPlatform(base.Add(time.Second), PlatformDeepseek))
	require.Equal(t, 0, circuit.activeBlockCountForPlatform(base.Add(time.Second), PlatformOpenAI))
}
...
func TestOpenAIProxyStreamCircuitSuccessDoesNotClearAnotherPlatform(t *testing.T) {
	...
	tripped, _ := circuit.recordFailureForPlatform(7, PlatformDeepseek, base)
	require.True(t, tripped)
	require.False(t, circuit.recordSuccessForPlatform(7, PlatformOpenAI))
	require.True(t, circuit.isBlockedForPlatform(7, PlatformDeepseek, base.Add(time.Second)))
	require.True(t, circuit.recordSuccessForPlatform(7, PlatformDeepseek))
	require.False(t, circuit.isBlockedForPlatform(7, PlatformDeepseek, base.Add(time.Second)))
}

func TestOpenAIProxyStreamCircuitActiveBlockCountDeduplicatesSharedProxy(t *testing.T) {
	base := time.Unix(1_800_000_000, 0)
	circuit := newOpenAIProxyStreamCircuit(openAIProxyStreamCircuitSettings{
		failureThreshold: 1,
		failureWindow:    time.Minute,
	...
	require.True(t, tripped)
	require.Equal(t, 1, circuit.activeBlockCount(base.Add(time.Second)))
	require.Equal(t, 1, circuit.activeBlockCountForPlatform(base.Add(time.Second), PlatformDeepseek))
	require.Equal(t, 1, circuit.activeBlockCountForPlatform(base.Add(time.Second), PlatformOpenAI))
}
...
func TestOpenAIDirectAPIKeyStreamDisconnectUsesAccountCircuit(t *testing.T) {
	...
	require.False(t, svc.isOpenAIProxyStreamQuarantined(context.Background(), account), "timeouts are not counted as stream disconnects")
	...
	require.False(t, svc.isOpenAIProxyStreamQuarantined(context.Background(), account), "a successful stream clears the direct-account quarantine")
}
