...
// CursorCredentialUnavailableClientMessage is the stable, non-sensitive message
// returned to clients when no healthy Cursor account credential is available.
const CursorCredentialUnavailableClientMessage = "No healthy Cursor account is currently available"

// CursorClientVersionRejectedClientMessage names the knob an operator has to
// change. This failure is identical on every account in the pool, so the
// message has to say so or it reads as an account problem.
...
const (
	...
	// CursorCredentialReasonWebSession marks an account still holding a browser
	// cookie: authentic, accepted by AvailableModels, rejected by chat.
	CursorCredentialReasonWebSession GatewayFailureReason = "cursor_credential_web_session"
	// CursorCredentialReasonClientVersion marks a permission_denied that is
	// about the advertised client version, not the credential.
	CursorCredentialReasonClientVersion GatewayFailureReason = "cursor_client_version_rejected"
)

// cursorCredentialFailureSnapshotError preserves the credential identity at the
// point of failure. Cursor has no proxy-bound CAS mutation contract like Grok,
// so the snapshot is currently informational (kept for symmetry / future use).
...
// classifyCursorCredentialFailure maps a credential-acquisition error onto a
// failover classification. Missing/expired credentials retry the next account;
// provider misconfiguration stops the failover loop.
...
// newCursorCredentialFailover builds an UpstreamFailoverError for a Cursor
// credential failure and records a sanitized ops event.
