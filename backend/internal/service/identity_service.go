...
func defaultFingerprint() Fingerprint {
	return Fingerprint{
		// Use the runtime-resolved version so a newly synchronized Claude CLI
		// version is reflected for newly created identities as well as in the
		// billing attribution block.  Keeping this on EffectiveCLIVersion avoids
		// a split-brain UA (runtime version in one path, compiled baseline here).
