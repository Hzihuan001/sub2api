...
func (s *ContentModerationService) UpdateConfig(ctx context.Context, input UpdateContentModerationConfigInput) (*ContentModerationConfigView, error) {
	...
	// The flat connection fields are the legacy OpenAI profile.  When the
	// selected engine is TypeSafe, updateEngineProfile below owns these
	// fields; mutating the legacy top-level values here would overwrite the
	// persisted OpenAI draft while switching engines.
