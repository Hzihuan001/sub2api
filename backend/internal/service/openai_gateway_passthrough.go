...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	// Resolve the local writer only after installing the persistent wrapper;
	// otherwise heartbeat and request writes could bypass its mutex.
	w := c.Writer
				...
				if !outputStarted {
					...
					if !cyberHit && !sawBareError {
						if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(c, account.Platform, dataBytes, failedMessage); matched {
							...
							// Stop the heartbeat before this early JSON response takes
							// over the wrapped writer and headers.
							stopKeepalive()
							MarkResponseCommitted(c)
