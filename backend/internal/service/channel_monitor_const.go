...
const (
	...
	// 代理站监控默认使用 SSE 探测，主站会持续发送心跳；45 秒会在
	// 首 token 较慢但连接仍健康时制造假失败，因此给监控留出 3 分钟。
	monitorRequestTimeout = 180 * time.Second
	...
	// monitorResponseHeaderTimeout HTTP transport 等待响应头超时。流式主站
	// 正常会很快返回响应头，90 秒只覆盖瞬时排队，不改变用户网关超时。
