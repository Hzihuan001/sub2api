...
func (s *adminServiceImpl) CreateUser(ctx context.Context, input *CreateUserInput) (*User, error) {
	...
	// 角色可由超级管理员在创建时指定(admin/manager/user);未提供时默认 user。
	...
	// 创建后台管理角色属权限敏感操作，落审计日志（含操作者），便于事后追溯。
	if user.Role == RoleAdmin || user.Role == RoleManager {
		logger.LegacyPrintf("service.admin", "audit: administrative user created actor_admin_id=%d target_user_id=%d target_role=%s",
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	// 角色变更(admin/manager/user);空字符串表示不修改。
	if input.Role != "" {
