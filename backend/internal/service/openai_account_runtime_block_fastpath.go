...
func (s *OpenAIGatewayService) handleOpenAIAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, canonicalModel ...string) bool {
	...
	// OpenAI-compatible API-key providers (DeepSeek/Kimi/Zhipu/MiniMax/
	// OpenCodeGo) use the same gateway and scheduler as OpenAI. A transient
	// 5xx/52x from one provider endpoint must therefore enter the existing
	// account+model runtime breaker as well; otherwise a single bad upstream is
	// selected again for every request until its CDN timeout expires.
