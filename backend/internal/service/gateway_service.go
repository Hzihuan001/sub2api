...
// mixedListingAccountAllowed mirrors mixed scheduling: a Gemini group may be
// served by Antigravity accounts, so model discovery must include those
// accounts just like the request scheduler does.
func mixedListingAccountAllowed(groupPlatform string, account *Account) bool {
	return groupPlatform == PlatformGemini && account != nil && account.IsMixedSchedulingEnabled()
}

// mixedListingModelAllowed limits models contributed by a mixed account to the
// wire IDs that belong to the requested platform.
