...
func buildAccountForCreate(input *CreateAccountInput, accountExtra map[string]any) (*Account, error) {
	...
	// OpenCode Go usage state is populated by the background usage worker and
	// is never client-controlled at account creation time.  In particular, do
	// not allow a crafted create payload to seed a trusted snapshot or enable
	// automatic refresh before the account has been verified by that worker.
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		// Usage state for an eligible OpenCode Go account is managed by the
		// background refresher.  Preserve the database snapshot for an
		// unrelated edit, while identity changes are intentionally handled by
		// the invalidation block below.  This also keeps the service-layer
		// contract safe for repositories that do not implement the optional
		// lock-and-merge extension.
		if IsOpenCodeGoUsageAccount(account) && reflect.DeepEqual(previousOpenCodeUsageIdentity, openCodeGoUsageIdentity(account)) {
